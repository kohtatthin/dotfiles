...
if is_windows then
  launcher_apps = {
    ...
    { id = 'codex',      label = 'Codex CLI (会社)',    cmd = 'cd C:\\claude; codex' },
    { id = 'codex-personal', label = 'Codex CLI (個人)', cmd = '$env:CODEX_HOME = "$HOME\\.codex-personal"; codex' },
    { id = 'fugu',       label = '🐟 Sakana Fugu',      cmd = 'cd C:\\claude; codex --profile sakana-fugu' },
    { id = 'fugu-ultra', label = '🐡 Sakana Fugu Ultra', cmd = 'cd C:\\claude; codex --profile sakana-fugu-ultra' },
...
-- Codex(5h/週) の使用率を表示する。重い ccusage 算出は ai_usage.ps1 が
-- バックグラウンドに逃がすので、ここでは結果を間引いて取得するだけ。
if is_windows then
  local ai_status = ''
  local ai_last = 0
  local AI_INTERVAL = 30  -- 取得間隔（秒）
  wezterm.on('update-status', function(window, pane)
    local now = os.time()
    if ai_status == '' or (now - ai_last) >= AI_INTERVAL then
      ai_last = now
